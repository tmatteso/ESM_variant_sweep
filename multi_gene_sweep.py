...
def compute_metrics(X_and_ys):
    ...
    for i in range(0, len(path_name_ls), 2):
        ...
        #print(y.shape)
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=42)
        print("ESM kNN ROC-AUC:", classifier_full_split(X, y, X_train, X_test, y_train, y_test))
        print("ESM", path_name_ls[i], path_name_ls[i+1], over_clf_space(X, y, "classifier_full_split"))
    
    # then compute roc-auc from VAE for clinvar only
    X = np.stack(X_and_ys[0]["VAE_embed"].values)#.reshape(X_and_ys[0]["LLR"].to_numpy().shape[0], 1)
    y = X_and_ys[1].to_numpy()
    #print(X.shape)
    #print(y.shape)
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=42)
    print("Clinvar VAE ROC-AUC:", classifier_full_split(X, y, X_train, X_test, y_train, y_test))
    # then compute roc-auc from UMAP for clinvar only
    X = np.stack(X_and_ys[0]["umap_embed"].values)#.reshape(X_and_ys[0]["LLR"].to_numpy().shape[0], 1)
    y = X_and_ys[1].to_numpy()
    #print(X.shape)
    #print(y.shape)
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=42)
    print("Clinvar UMAP ROC-AUC:", classifier_full_split(X, y, X_train, X_test, y_train, y_test))
def main():
    name_ls = [
